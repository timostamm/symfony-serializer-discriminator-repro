 ...
 */
abstract class Patched_AbstractObjectNormalizer extends AbstractNormalizer
{
    ...
    /**
     * Resolves the actual class if a discriminator mapping exists.
     *
     * Silently returns null if no class discriminator resolver is
     * present or no mapping exists.
     *
     * @param string $class
     * @param array $data
     * @param string|null $format
     ...
     * @return null|string
     */
    protected function resolveDiscriminatedClass(string $class, array & $data, string $format = null, array & $context): ?string
    {
        ...
        if (!isset($data[$discriminatorProperty])) {
            throw new RuntimeException(sprintf('Type property "%s" is missing for the abstract object "%s"', $discriminatorProperty, $class));
        }

        $discriminatorType = $data[$discriminatorProperty];
        if (!is_string($discriminatorType)) {
            throw new RuntimeException(sprintf('Type property "%s" is not a string for the abstract object "%s"', $discriminatorProperty, $class));
        }

        $resolvedClass = $mapping->getClassForType($discriminatorType);
        if (null === $resolvedClass) {
            throw new RuntimeException(sprintf('The type "%s" has no mapped class for the abstract object "%s"', $discriminatorType, $class));
        }
